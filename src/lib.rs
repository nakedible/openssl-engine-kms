...
type pkey_init_fn = extern fn(EVP_PKEY_CTX) -> c_int;
type pkey_sign_fn = extern fn(EVP_PKEY_CTX, *mut c_uchar, *mut usize, *const c_uchar, usize) -> c_int;
type pkey_verify_fn = extern fn(EVP_PKEY_CTX, *const c_uchar, usize, *const c_uchar, usize) -> c_int;
type pkey_encrypt_fn = extern fn(EVP_PKEY_CTX, *mut c_uchar, *mut usize, *const c_uchar, c_int) -> c_int;
type pkey_decrypt_fn = extern fn(EVP_PKEY_CTX, *mut c_uchar, *mut usize, *const c_uchar, c_int) -> c_int;
...
extern fn load_key(e: ENGINE, key_id: *const c_char, _ui_method: *mut c_void, _callback_data: *mut c_void) -> EVP_PKEY {
  ...
  unsafe {
    let key_bio = openssl_try!(BIO_new_mem_buf(bytes.as_ptr() as *const c_void, bytes.len() as c_int), ptr::null_mut(), ptr::null_mut());
